module_name = 'test_17_PIO_D_read_PWM_v04.py'
print (module_name)
...
for i in range(100):  #  arbitrary test duration
    ...
    for i in range(len(sm_list)):
        ...
        if sm.rx_fifo():
            y = sm.get()
            if ((y < (previous_list[i] - tolerance)) or (y > (previous_list[i] + tolerance))):
                print ('sm',i,y)
                previous_list[i] = y
